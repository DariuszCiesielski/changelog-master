...
export function SourcesPanel() {
  ...
  const handleAddSource = async () => {
    if (!newName.trim() || !newUrl.trim()) {
      setAddError(t('sources.nameUrlRequired'));
      return;
    }
        ...
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl bg-white dark:bg-charcoal-800 rounded-xl shadow-2xl z-50 p-6 max-h-[90vh] overflow-y-auto transition-colors duration-500">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-charcoal-900 dark:text-cream-50">{t('sources.title')}</h2>
              <button
                onClick={() => setIsOpen(false)}
              ...
              <div className="space-y-4">
                      ...
                      {editingId === source.id ? (
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            placeholder={t('sources.changelogUrl')}
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0 flex-1">
                              ...
                              {source.last_version && (
                                <p className="text-xs text-charcoal-500 dark:text-charcoal-400 mt-1">
                                  {t('sources.lastVersion')} <span className="font-mono">{source.last_version}</span>
                                </p>
                              )}
                            </div>
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                title={source.is_active ? t('sources.disableSource') : t('sources.enableSource')}
                              >
                ...
                {showAddForm ? (
                  <div className="p-4 border-2 border-dashed border-coral-400/30 dark:border-coral-600/30 rounded-xl bg-coral-400/10 dark:bg-coral-600/10 space-y-4">
                    <h3 className="font-medium text-charcoal-900 dark:text-cream-50">{t('sources.addNewSource')}</h3>
                    ...
                    <div>
                      <label className="block text-sm text-charcoal-600 dark:text-charcoal-400 mb-1">{t('sources.name')}</label>
                      <input
                        type="text"
                        value={newName}
                    ...
                    <div>
                      <label className="block text-sm text-charcoal-600 dark:text-charcoal-400 mb-1">{t('sources.changelogUrl')}</label>
                      <div className="flex gap-2">
                        <input
                          type="url"
                          value={newUrl}
                        ...
                        {testResult.valid ? (
                          <div className="space-y-2">
                            <div className="flex items-center gap-2 text-teal-700 dark:text-teal-400">
                              ...
                              <span>{t('sources.validChangelog')}</span>
                            </div>
                            <p className="text-charcoal-600 dark:text-charcoal-400">
                              {t('sources.latestVersion')} <span className="font-mono font-medium">{testResult.latestVersion}</span>
                            </p>
                            {testResult.preview && (
                              <details className="text-xs">
                                <summary className="cursor-pointer text-charcoal-500 hover:text-charcoal-700 dark:hover:text-cream-200">
                                  {t('sources.previewContent')}
                                </summary>
